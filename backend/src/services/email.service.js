...
export const sendEmail = async ({ to, subject, text, html, customConfig = null }) => {
  try {
    ...
    if (transporter) {
      const mailOptions = {
        from: customConfig?.smtpFrom || process.env.EMAIL_FROM || '"Dsofts IT CRM" <crm@dsoftsit.com>',
        to,
        subject,
        text,
        html
...
export const testSmtpConnection = async (smtpConfig, recipientEmail) => {
  ...
  const mailOptions = {
    from: smtpConfig?.smtpFrom || '"Dsofts IT CRM" <crm@dsoftsit.com>',
    to: recipientEmail,
    subject: "Dsofts IT CRM - SMTP Test Email",
    text: "Your SMTP configuration is verified and functioning perfectly with Dsofts IT CRM.",
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 540px; margin: auto; padding: 25px; border: 1px solid #e2d8c0; border-radius: 16px; background-color: #FAF9F5;">
        <div style="text-align: center; margin-bottom: 20px;">
          <h2 style="color: #2563eb; margin: 0; font-size: 22px; font-weight: 800; letter-spacing: 1px;">DSOFTS IT SUITE</h2>
        ...
        <div style="background: white; padding: 20px; border-radius: 12px; border: 1px solid #eee; text-align: center;">
          ...
          <p style="color: #44403c; font-size: 13px; line-height: 1.6; margin: 0;">
            Your email server credentials have been validated. Dsofts IT CRM is now configured to send automated client assignment and administrative notifications.
          </p>
        </div>
        <p style="color: #a8a29e; font-size: 11px; text-align: center; margin-top: 20px;">Sent from Dsofts IT CRM System Engine</p>
      </div>
    `
  };
...
export const sendStaffAssignmentEmail = async ({ staffEmail, staffName, clientName, clientPhone, enquiryNo, projectType, budget, assignedBy }) => {
  ...
  const subject = `[Dsofts IT CRM] New Client Assigned: ${clientName} (${enquiryNo || "New Lead"})`;
  const text = `Hello ${staffName},\n\nYou have been assigned to handle a new client enquiry in Dsofts IT CRM:\n\nClient Name: ${clientName}\nPhone: ${clientPhone || "N/A"}\nEnquiry No: ${enquiryNo}\nProject: ${projectType || "Standard"}\nBudget: ${budget || "Standard"}\nAssigned By: ${assignedBy || "Admin"}\n\nPlease review the enquiry in your Dsofts IT CRM dashboard.`;
  ...
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 580px; margin: auto; padding: 25px; border: 1px solid #EAE3D2; border-radius: 16px; background-color: #FAF9F5;">
      <div style="border-bottom: 2px solid #2563eb; padding-bottom: 15px; margin-bottom: 20px;">
        <h2 style="color: #2563eb; margin: 0; font-size: 20px; font-weight: 800;">DSOFTS IT CRM</h2>
      ...
      <p style="color: #44403c; font-size: 13px; line-height: 1.5;">You have been assigned as the primary handler for a new client in Dsofts IT CRM.</p>
...
export const sendAdminActivityNotification = async ({ actionType, performedBy, role, clientName, details, timestamp = new Date() }) => {
  let adminEmail = "admin@dsoftsit.com";
  try {
    const settings = await CompanySetting.findOne();
  ...
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: auto; padding: 22px; border: 1px solid #EAE3D2; border-radius: 14px; background-color: #FAF9F5;">
      <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid #2563eb; padding-bottom: 12px; margin-bottom: 16px;">
        <h3 style="color: #2563eb; margin: 0; font-size: 18px; font-weight: 800;">DSOFTS IT AUDIT ALERT</h3>
      ...
      <div style="background: white; border: 1px solid #EAE3D2; border-radius: 10px; padding: 16px;">
        ...
        <div style="font-size: 12px; color: #44403c; line-height: 1.6;">
          <p style="margin: 4px 0;"><strong>Operator:</strong> ${performedBy} <span style="color: #2563eb; font-weight: 700;">(${role || "Staff"})</span></p>
      ...
      <p style="color: #a8a29e; font-size: 10px; text-align: center; margin-top: 16px;">Dsofts IT ERP Automated Audit Dispatch</p>
    </div>
  `;
...
export const sendPasswordResetEmail = async ({ to, resetUrl, userName = "User" }) => {
  const subject = "Password Reset Request - Dsofts IT ERP";
  const text = `Hello ${userName},\n\nYou requested a password reset for your Dsofts IT ERP account. Click the link below or copy it to your browser to set a new password:\n\n${resetUrl}\n\nIf you did not make this request, you can safely ignore this email.\n\nDsofts IT Suite`;
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: auto; padding: 24px; border: 1px solid #EAE3D2; border-radius: 14px; background-color: #FAF9F5;">
      <h2 style="color: #2563eb; margin: 0 0 16px 0; font-weight: 800;">Password Reset Request</h2>
      <p style="color: #292524; font-size: 14px;">Hello <strong>${userName}</strong>,</p>
      <p style="color: #57534e; font-size: 13px;">You requested a password reset for your Dsofts IT ERP account. Click the button below to choose a new password:</p>
      <div style="text-align: center; margin: 24px 0;">
        <a href="${resetUrl}" style="display: inline-block; padding: 12px 24px; background-color: #2563eb; color: white; text-decoration: none; border-radius: 8px; font-weight: 700; font-size: 13px;">Reset Password</a>
      </div>
...
export const sendOtpEmail = async ({ to, otp, userName = "Customer" }) => {
  const subject = `Your Dsofts IT Verification Code: ${otp}`;
  const text = `Hello ${userName},\n\nYour Dsofts IT verification OTP code is: ${otp}.\n\nThis code will expire in 10 minutes.\n\nDsofts IT Suite`;
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: auto; padding: 24px; border: 1px solid #EAE3D2; border-radius: 14px; background-color: #FAF9F5;">
      <h2 style="color: #2563eb; margin: 0 0 16px 0; font-weight: 800;">Verification Code</h2>
      <p style="color: #292524; font-size: 14px;">Hello <strong>${userName}</strong>,</p>
      ...
      <div style="text-align: center; margin: 24px 0;">
        <span style="display: inline-block; padding: 12px 28px; background-color: #EFF6FF; border: 2px solid #2563eb; color: #2563eb; font-family: monospace; font-size: 24px; font-weight: 900; letter-spacing: 6px; border-radius: 8px;">${otp}</span>
      </div>
